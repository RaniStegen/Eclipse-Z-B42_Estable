...
FancyHands.doorDecide = function(obj, playerObj, delayed, fromClick)
    ...
    if obj:getProperties():Is("GarageDoor") then
        -- For garage doors, we want to use the dual-hand animation (FH_Boop_Dual)
        -- which triggers when extra == 1
        -- Garage doors are large and typically require both hands to open/close
        -- Use high position (101) for the animation
...
Events.OnGameStart.Add(function()
    -- This file is shared, so it also runs on a dedicated server, where the
    -- keybind system (and therefore FancyHands.addKeyBind) never gets built.
    if isServer() or not FancyHands.addKeyBind then return end
