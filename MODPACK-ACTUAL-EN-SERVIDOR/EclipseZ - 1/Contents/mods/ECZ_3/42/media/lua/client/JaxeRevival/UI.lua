...
local onFillWorldObjectContextMenu = function(playerNum, context)
  ...
  local clickedPlayer = ISWorldObjectContextMenu.fetchVars.clickedPlayer
  ...
  if SandboxVars.JaxeRevival.FirstAidRequired > 0 and SandboxVars.JaxeRevival.FirstAidRequired > player:getPerkLevel(Perks.Doctor) then
    local option = context:addOptionOnTop(getText("ContextMenu_JaxeRevival_ActionFirstAidRequired", clickedName, SandboxVars.JaxeRevival.FirstAidRequired))
  ...
  elseif not JaxeRevival.Skills.canProfessionRevive(player) then
    ...
    context:addOptionOnTop(getText("ContextMenu_JaxeRevival_Action", clickedName), player, onReviveAction, clickedPlayer)
  end
end
