...
local TOOLTIP_INFO_PREFIX = "Tooltip_MarzGuns_Info_"

local function localizeTooltipLine(line)
    if type(line) ~= "string" then return line end

    if string.sub(line, 1, #TOOLTIP_INFO_PREFIX) == TOOLTIP_INFO_PREFIX then
        local translated = getText(line)
...
local function normalizeTooltipLines(entry)
    if type(entry) == "string" then
        ...
        for line in string.gmatch(entry, "[^\r\n]+") do
            if line ~= "" then
                lines[#lines + 1] = localizeTooltipLine(line)
            end
        end
...
local function appendCustomTooltipBlock(tooltip, lines)
    ...
    layout:addItem():setLabel(getText("Tooltip_MarzGuns_Information"), 1, 0.02, 0.02, 1)
