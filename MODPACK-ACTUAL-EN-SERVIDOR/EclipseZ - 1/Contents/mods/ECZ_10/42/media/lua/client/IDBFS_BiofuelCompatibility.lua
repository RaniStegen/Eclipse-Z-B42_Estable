...
local function getContainerFuelType(item)
    if isBiofuelContainer(item) then
        return "biofuel"
    end
    if hasPetrolContainer(item) then
        return "petrol"
    end
    return nil
end
...
local function getVehiclePartFuelType(part)
    local amount = part and part.getContainerContentAmount and tonumber(part:getContainerContentAmount()) or 0
    if amount <= 0 then
        return nil
    end
...
local function getFuelContainers(playerObj, requiredFuelType)
    local inventory = playerObj and playerObj:getInventory() or nil
    if not inventory then
        return nil
    end
    return inventory:getAllEvalRecurse(function(item)
        if not isUsableEngineFuel(item) then
            return false
        end
        return not requiredFuelType or getContainerFuelType(item) == requiredFuelType
    end)
end
...
local function addGeneratorFuelOptions(worldobjects, generator, player, context)
    ...
    local pourOut = getFuelContainers(playerObj, getGeneratorFuelType(generator))
...
local function vehicleAddFuelMenuPatch(playerObj, part, context)
    ...
    local playerNum = playerObj:getPlayerNum()
    local pourOut = getFuelContainers(playerObj, getVehiclePartFuelType(part))
    if not pourOut or pourOut:isEmpty() then
        return
    end
