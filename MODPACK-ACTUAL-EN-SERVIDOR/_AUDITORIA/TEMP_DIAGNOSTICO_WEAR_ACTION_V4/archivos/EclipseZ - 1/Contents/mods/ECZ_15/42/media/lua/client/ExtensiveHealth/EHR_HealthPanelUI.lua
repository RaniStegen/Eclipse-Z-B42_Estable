...
function EHR.UI.ShouldHeartButtonOpenEHR()
    -- Eclipse Z: el corazón es siempre la entrada principal al panel de EHR.
    -- No se deja esta decisión en ModOptions para evitar que una configuración
    -- antigua vuelva a abrir el panel médico vanilla.
    return true
end
...
local function patchEquippedItemHealthButton()
    ...
    local currentOnOptionMouseDown = ISEquippedItem.onOptionMouseDown
    if currentOnOptionMouseDown ~= EHR.UI.EquippedItemHealthMouseDownWrapper then
        EHR.UI.OriginalEquippedItemHealthMouseDown = currentOnOptionMouseDown

        local originalOnOptionMouseDown = currentOnOptionMouseDown
        local healthMouseDownWrapper = function(self, button, x, y)
            if button and button.internal == "HEALTH" and EHR.UI and EHR.UI.ToggleHealthPanel then
                local playerNum = self and self.playerNum or 0
                local player = self and self.chr
                    or (getSpecificPlayer and getSpecificPlayer(playerNum))
        ...
        EHR.UI.EquippedItemHealthMouseDownWrapper = healthMouseDownWrapper
        ISEquippedItem.onOptionMouseDown = healthMouseDownWrapper
    end

    local originalPrerender = ISEquippedItem.prerender
    if originalPrerender and originalPrerender ~= EHR.UI.EquippedItemHealthPrerenderWrapper then
        local healthPrerenderWrapper = function(self)
            originalPrerender(self)
            if self and self.healthBtn and self.heartIconOn and self.heartIconOff and EHR.UI then
                if EHR.UI.HealthPanelInstance and EHR.UI.HealthPanelInstance:isVisible() then
        ...
        EHR.UI.EquippedItemHealthPrerenderWrapper = healthPrerenderWrapper
        ISEquippedItem.prerender = healthPrerenderWrapper
    end

    -- Los ISButton guardan la función al crearse. Reasigna también el botón
    -- existente para cubrir interfaces creadas antes de cargar EHR.
    local equippedItem = ISEquippedItem.instance
    if equippedItem and equippedItem.healthBtn and EHR.UI.EquippedItemHealthMouseDownWrapper then
        equippedItem.healthBtn.target = equippedItem
        equippedItem.healthBtn.onclick = EHR.UI.EquippedItemHealthMouseDownWrapper
    end
...
if Events and Events.OnCreatePlayer then
    Events.OnCreatePlayer.Add(patchEquippedItemHealthButton)
end
