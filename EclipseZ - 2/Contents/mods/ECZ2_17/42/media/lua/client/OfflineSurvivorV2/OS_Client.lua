...
local function addOfflineContext(playerIndex, context, worldObjects, test)
    ...
    local label = context:addOption((data.username or "Player") .. " is resting.", nil, nil)
    label.notAvailable = true

    local option = context:addOption("Search", OS, OS.requestLoot, playerIndex, data.steamId)
