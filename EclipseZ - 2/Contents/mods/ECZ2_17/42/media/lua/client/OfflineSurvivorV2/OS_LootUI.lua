...
function OS.LootUI:createChildren()
    ...
    self.addButton = ISButton:new(362, 155, 96, 25, "Add >", self, OS.LootUI.onAdd)
    self.addButton:initialise()
    self:addChild(self.addButton)

    self.removeButton = ISButton:new(362, 190, 96, 25, "< Remove", self, OS.LootUI.onRemove)
    self.removeButton:initialise()
    self:addChild(self.removeButton)

    self.stealButton = ISButton:new(604, self.height - 42, 122, 25, "Steal", self, OS.LootUI.onSteal)
    self.stealButton:initialise()
    self:addChild(self.stealButton)

    self.cancelButton = ISButton:new(736, self.height - 42, 72, 25, "Cancel", self, OS.LootUI.onCloseButton)
...
function OS.LootUI:setLootData(args)
    ...
    self.targetName = args and args.targetName or "Player"
...
function OS.LootUI:refreshLists()
    ...
    self.stealButton.title = "Steal (" .. selectedCount .. "/" .. self.maxItems .. ")"
end
...
function OS.LootUI:render()
    ...
    self:drawText("Available Items", 12, 38, 1, 1, 1, 1, UIFont.Small)
    self:drawText("Selected Items", 470, 38, 1, 1, 1, 1, UIFont.Small)
    self:drawText("Choose up to " .. tostring(self.maxItems or 3) .. " item(s) from " .. tostring(self.targetName or "Player") .. ".", 12, self.height - 70, 1, 1, 1, 1, UIFont.Small)
end
...
function OS.LootUI:new(x, y)
    ...
    self.__index = self
    object.title = "Search Offline Survivor"
