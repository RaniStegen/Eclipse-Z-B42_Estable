...
function SL.LoadSettings()
    ...
    SL.Settings = {
        ...
        MusicBonus = math.floor(numberValue(vars.MusicBonus, DEFAULTS.MusicBonus, 0, 3)),
...
local function categoryOption(perkObject, perkId)
    if MUSIC_IDS[normalized(perkId)] then return "MusicBonus", true end
...
function SL.GetTierPoints(character, perkObject, perkId)
    ...
    total = total + traitBoost(character, perkId)
    total = total + professionBoost(character, perkId)
    return total
end
...
function SL.GetMaxSkill(character, perk)
    ...
    if points == nil then return cfg.PerkLvl3Cap end
