...
function HairMenuPanel:render()
    ...
    local x_left = self.pageRightButton:getRight() + 10

    -- pages
    local pages  = self.pageCurrent .. "/" .. self:getNumberOfPages()
    self:drawText(pages, x_left, h/2, 0.9, 0.9, 0.9, 0.9, UIFont.Small)
    x_left = x_left + 40

    -- available width until right mini controls begin
    local rightW = 0
    local gap    = 8
    local ctrl   = self.hairControlsBtn or self.beardControlsBtn
    if self.stubbleTickBox then rightW = rightW + self.stubbleTickBox:getWidth() + gap end
    if self.hairColorBtn   then rightW = rightW + self.hairColorBtn:getWidth()   + gap end
    if ctrl                then rightW = rightW + ctrl:getWidth()                + gap end

    local x_rightLimit = self:getWidth() - rightW - 10
    local maxTitleW    = math.max(0, x_rightLimit - x_left)

    if self.showSelectedName and self.selectedHairInfo and type(self.selectedHairInfo) == "table" then
        local title = _IHM_clipToWidth(UIFont.Small, self.selectedHairInfo.display or "", maxTitleW)
        self:drawText(title, x_left, h/2, 0.9, 0.9, 0.9, 0.9, UIFont.Small)
    end
end
