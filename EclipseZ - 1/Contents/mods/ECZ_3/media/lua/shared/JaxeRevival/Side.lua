...
JaxeRevival.Side.COMMAND_REPORT_INCAPACITATE = "REPORT_INCAPACITATE"
...
JaxeRevival.Side.canUseAdminCommands = function(player) return isAdmin(player) end
...
JaxeRevival.Side.reportIncapacitate = function(value) sendToServer(JaxeRevival.Side.COMMAND_REPORT_INCAPACITATE, { target = getPlayer():getUsername(), value = value or false }) end
...
local onServerCommand = function(module, command, args)
  ...
  elseif target and args.initiator ~= getPlayer():getUsername() then
    if command == JaxeRevival.Server.COMMAND_SEND_INCAPACITATE then
      JaxeRevival.Incapacitation.apply(target, args.value, true)
    elseif command == JaxeRevival.Server.COMMAND_SEND_REVIVE then
      JaxeRevival.Health.revive(target, args.bandage, args.restore, true)
    elseif command == JaxeRevival.Server.COMMAND_SEND_KILL then
      JaxeRevival.Health.kill(target, true)
    end
  end
end
