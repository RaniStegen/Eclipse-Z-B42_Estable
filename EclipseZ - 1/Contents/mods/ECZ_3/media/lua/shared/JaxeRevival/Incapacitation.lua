...
JaxeRevival.Incapacitation.apply = function(player, value, serverInitiated)
  ...
  if initial or not value then hasReported = nil end
  ...
  if JaxeRevival.Side.isClientOrSingleplayer() then
    ...
    if not serverInitiated and not hasReported then
      JaxeRevival.Side.reportIncapacitate(value)
      hasReported = true
    end
  end
  ...
  if value then
    if initial then
      if JaxeRevival.Side.isClientOrSingleplayer() and not JaxeRevival.Skills.checkPassives(player) then
        JaxeRevival.Health.kill(player)
        return
      end
    ...
    if remaining and JaxeRevival.Side.isClientOrSingleplayer() and remaining <= 0 then conclude(player) end
  else
