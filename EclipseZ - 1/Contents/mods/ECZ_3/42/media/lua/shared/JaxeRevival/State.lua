...
JaxeRevival.State.getTimeRemainingText = function(player, value)
  ...
  if SandboxVars.JaxeRevival.ShowExactCountdown then
    ...
    local exact = ""
    exact = getTimeUnit(exact, math.floor(total / 24), "IGUI_Gametime_day", "IGUI_Gametime_days")

    local remaining = total % 24
    exact = getTimeUnit(exact, math.floor(remaining), "IGUI_Gametime_hour", "IGUI_Gametime_hours")
    exact = getTimeUnit(exact, math.floor((remaining * 60) % 60), "IGUI_Gametime_minute", "IGUI_Gametime_minutes")
    if #exact == 0 then exact = getTimeUnit(exact, math.floor((remaining * 3600) % 60), "IGUI_Gametime_second", "IGUI_Gametime_secondes") end

    text = getText("UI_JaxeRevival_RemainingExact", exact)
  else
    if total < 1 then
  ...
  return getText(JaxeRevival.Incapacitation.canRecoverUnassisted(player) and "UI_JaxeRevival_WillRecover" or "UI_JaxeRevival_WillDie", text)
end
...
local onZombieUpdate = function(zombie)
  ...
  if target and instanceof(target, "IsoPlayer") and not JaxeRevival.Incapacitation.isActive(target) then return end
  zombie:setEatBodyTarget(nil, true)
