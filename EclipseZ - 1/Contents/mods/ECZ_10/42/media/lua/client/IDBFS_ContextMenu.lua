...
local function getWaterUnitsForItem(item)
    ...
    if fluidContainer and not fluidContainer:isEmpty() then
        ...
        if primary and WATER_FLUIDS[primary:getFluidTypeString()] == true then
            return math.floor(fluidContainer:getPrimaryFluidAmount())
        end
    end
...
local function hasWater(playerObj)
...
local function countMatching(playerObj, allowed)
    local count = 0
...
local function hasPressIngredients(playerObj, recipe)
    for _, req in ipairs(recipe.ingredients or {}) do
        if countMatching(playerObj, req.anyOf) < req.count then
            return false
        end
    end
    if recipe.needsWater and not hasWater(playerObj) then
        return false
    end
    local containers = collectItems(playerObj, function(item)
        return IDBFS.isPressContainer(item) and IDBFS.canContainerAcceptLiquid(item, recipe.liquidType, 0.001)
    end)
    return #containers > 0
end
...
local function addPressOptions(subMenu, playerObj, obj)
    ...
    for key, recipe in pairs(IDBFS.PressRecipes or {}) do
        ...
        local option = subMenu:addOption(label, nil, function()
            local containers = collectPressContainers(playerObj, recipe.liquidType, 0.001)
            sortItemsByHand(playerObj, containers)
            local container = containers[1]
            queueCommand(playerObj, obj, "press", { recipe = key, containerID = container and container:getID() or nil }, 180, container)
        end)
        if not hasPressIngredients(playerObj, recipe) then
            option.notAvailable = true
            addTooltip(option, text("Tooltip_IDBFS_NeedIngredients", "Requires ingredients, a bucket-type container, and water if listed."))
        end
    end
end
...
local function addDebugOption(subMenu, playerObj, obj)
    ...
    subMenu:addOption("[Debug] Fill Fermented Wash", nil, function()
