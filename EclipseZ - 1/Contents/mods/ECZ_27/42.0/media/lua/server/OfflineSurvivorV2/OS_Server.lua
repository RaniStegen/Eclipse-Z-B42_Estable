...
local function createOfflineCorpse(record, square)
    ...
    if not player then return nil, "el último aspecto visual del jugador no está disponible" end

    local ok, result = pcall(function()
        if not IsoPlayer or not IsoDeadBody or not ItemVisuals or not sendCorpse then
            error("IsoPlayer, IsoDeadBody, ItemVisuals o sendCorpse no está disponible")
        end
        ...
        if not surrogate then error("no se pudo crear el sustituto visual") end
        ...
        if not worn or not container then error("el sustituto no creó un contenedor visual") end
        ...
        if not corpse then error("el motor no creó el cuerpo desconectado") end
...
local function createOfflineCorpseObject(record)
    ...
    local square = getRecordSquare(record)
    if not square then
        print("[OfflineSurvivor V2] La casilla de desconexión no está cargada para " .. tostring(record.username))
        return nil
    end
    ...
    if not corpse then
        ...
        if not record.corpseFailureReported then
            ...
            print("[OfflineSurvivor V2] Falló el cuerpo nativo de " .. tostring(record.username) .. ": " .. record.corpseFailure)
        end
        return nil
    end
    ...
    local placementName = ({ bed = "cama", sofa = "sofá", floor = "suelo" })[record.placement]
        or tostring(record.placement or "suelo")
    print("[OfflineSurvivor V2] Se ha creado el cuerpo nativo de " .. tostring(record.username) .. " en " .. placementName)
    return corpse
end
...
local function markOffline(record, isTestBody)
    ...
    if shouldBlockAdminOfflineBody(record) then
        ...
        record.objectCreated = nil
        print("[OfflineSurvivor V2] Se ha omitido el cuerpo desconectado del administrador " .. tostring(record.username))
    else
        record.bodySuppressed = nil
        if isTestBody then
            print("[OfflineSurvivor V2] Se ha creado el cuerpo de prueba invisible/noclip del administrador " .. tostring(record.username))
        else
            print("[OfflineSurvivor V2] Jugador desconectado: " .. tostring(record.username))
        end
...
local function trackOnlinePlayer(player)
    ...
    if not lastHeartbeatLog[steamId] or nowSeconds() - lastHeartbeatLog[steamId] >= 10 then
        ...
        print("[OfflineSurvivor V2] El servidor está siguiendo al jugador activo " .. tostring(record.username))
    end
...
local function cooldownMessage(remainingMs)
    ...
    if hours > 0 then
        return "Podrás volver a registrar dentro de " .. hours .. " h " .. rest .. " min."
    end
    local minuteLabel = minutes == 1 and " minuto." or " minutos."
    return "Podrás volver a registrar dentro de " .. minutes .. minuteLabel
end
    ...
    for _, info in ipairs(record.lootItems or {}) do
        if info and info.itemId and not isPendingTheft(record, info.itemId) then
            result[#result + 1] = {
                ...
                displayName = tostring(info.displayName or "Objeto"),
                weight = numberOr(info.weight, 0),
                condition = numberOr(info.condition, 0),
                containerPath = tostring(info.containerPath or "Cuerpo"),
            }
        end
    end
    return result
...
local function requestLoot(player, args)
    if not OS.isEnabled() or OS.getOption("EnableLoot", true) == false then
        denyLoot(player, "El saqueo de supervivientes desconectados está desactivado.")
        return
    end
    ...
    if not looterSteamId or not targetSteamId or targetSteamId == "" or sameSteamId(looterSteamId, targetSteamId) then
        denyLoot(player, "Objetivo no válido.")
        return
    end

    local record = records[targetSteamId]
    if not record or not hasValidOfflineObject(record) then
        denyLoot(player, "Este superviviente desconectado ya no está disponible.")
        return
    end
    if not isNearRecord(player, record) then
        denyLoot(player, "Estás demasiado lejos para registrar a este superviviente.")
        return
    end
    if not runtimePlayers[targetSteamId] then
        denyLoot(player, "Este inventario solo puede registrarse mientras el servidor conserve en memoria al jugador desconectado.")
        return
    end
    ...
    if #items == 0 then
        denyLoot(player, "No hay objetos disponibles para robar.")
        return
    end
    ...
    sendTo(player, OS.COMMAND_OPEN_LOOT, {
        sessionId = sessionId,
        targetName = tostring(record.username or "Jugador"),
...
local function commitLoot(player, args)
    ...
    if not session or tonumber(session.id) ~= tonumber(requestedSessionId) then
        denyLoot(player, "La sesión de registro ha caducado. Abre de nuevo la lista.")
        return
    end

    if nowMs() > session.expiresAtMs then
        finishLoot(player, looterSteamId, session.id, false, "La sesión de registro ha caducado.", 0)
        return
    end
    if not OS.isEnabled() or OS.getOption("EnableLoot", true) == false then
        finishLoot(player, looterSteamId, session.id, false, "El saqueo de supervivientes desconectados está desactivado.", 0)
        return
    end
    ...
    if not record or not target or not hasValidOfflineObject(record) then
        finishLoot(player, looterSteamId, session.id, false, "Este superviviente desconectado ya no está disponible.", 0)
        return
    end
    if not isNearRecord(player, record) then
        finishLoot(player, looterSteamId, session.id, false, "Te has alejado demasiado del superviviente.", 0)
        return
    end
    ...
    if not requestedIds or #requestedIds == 0 then
        finishLoot(player, looterSteamId, session.id, false, "Selecciona al menos un objeto válido.", 0)
        return
    end
    ...
    if not targetInventory then
        finishLoot(player, looterSteamId, session.id, false, "El inventario del superviviente no está disponible ahora mismo.", 0)
        return
    end
    ...
    if taken <= 0 then
        finishLoot(player, looterSteamId, session.id, false, "Ninguno de los objetos seleccionados seguía disponible.", 0)
        return
    end
    ...
    persistentDataChanged()
    local itemLabel = taken == 1 and " objeto." or " objetos."
    finishLoot(player, looterSteamId, session.id, true, "Has robado " .. taken .. itemLabel, taken)
end
...
local function onInitGlobalModData()
    ...
    runtimePlayers = {}
    lootSessions = {}
    print("[OfflineSurvivor V2] Script del servidor iniciado.")
end
