...
local function silence(wielder, weapon)
    CALIBER        = {
        ...
        bullets_3030   = { SandboxVars.Firearms.SuppressorEffectiveness308, 'FirearmARSuppressed' },
    }
    ...
    if canon and canon:hasTag(FirearmsTags.firearmsSuppressor) then
        ...
        if weapon:getWeaponReloadType() == "revolver" then
            effectivness = effectivness +
                ((1 - effectivness) * (1 - EFFECTIVENESS[SandboxVars.Firearms.SuppressorEffectivenessRevolver]))
        end
