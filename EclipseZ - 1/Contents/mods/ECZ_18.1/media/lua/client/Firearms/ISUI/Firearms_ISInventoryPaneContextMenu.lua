...
ISInventoryPaneContextMenu.onChangefiremode = function(playerObj, weapon, newfiremode)
	if not weapon:hasTag("SemiShotgun") and not weapon:getType() == "SPAS12" then
		original_onChangefiremode(playerObj, weapon, newfiremode)
	end
	if "Auto" == newfiremode and weapon:getType() == "SPAS12" then
		if getDebug() then
			print("Auto")
		end
		weapon:setRackAfterShoot(false);
	elseif "Single" == newfiremode and weapon:getType() == "SPAS12"then
		if getDebug() then
			print("Single")
		end
