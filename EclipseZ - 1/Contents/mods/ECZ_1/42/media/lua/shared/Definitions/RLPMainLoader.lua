...
local function verifyRegistries()
    ...
    if RLP.CharacterProfession.MEDICO_FORENSE and RLP.CharacterTrait.AUTOPSY_SPECIALIST then
        print("RLP: - Traits and professions verified successfully")
        print("RLP: - Profession: Médico Forense (rlp:medicoforense)")
...
local function verifyClothingDefinitions()
    if ClothingSelectionDefinitions and ClothingSelectionDefinitions["rlp:medicoforense"] then
...
local function performFinalChecks()
    ...
    -- Verifica profissões
    if not RLP.CharacterProfession.MEDICO_FORENSE then
        table.insert(errors, "Médico Forense profession not registered")
    end
    ...
    if not ClothingSelectionDefinitions or not ClothingSelectionDefinitions["rlp:medicoforense"] then
