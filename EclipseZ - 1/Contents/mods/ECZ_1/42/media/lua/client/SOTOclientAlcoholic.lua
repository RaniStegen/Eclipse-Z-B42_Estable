...
local function SOTOAlcoholicMain()
	...
	-- slowly decrease SleepingTabletDelta to make alcohol resistance
		...
		if currentSTDelta > 0 then
		
		SOTOdecreaseSleepingTabletDelta(player, 100, 0.00015);
	...
	if AlcoRemovable then
		...
		SOTOremoveTrait(player, SOTO.CharacterTrait.ALCOHOLIC);
		SOTOaddTrait(player, SOTO.CharacterTrait.FORMER_ALCOHOLIC);	
...
-- WITHDRAWAL FX --
-----------------------
local function SOTOalcoholicEffects(player, ticks)
...
local function SOFormerAlcoholicRelapseCheck(player, rollback)
	...
	modData.FormerAlcoholicRelapseChance = math.min(100, (modData.FormerAlcoholicRelapseChance or 100) + rollback);
	...
	if roll < modData.FormerAlcoholicRelapseChance then
		...
		SOTOremoveTrait(player, SOTO.CharacterTrait.FORMER_ALCOHOLIC);
		SOTOaddTrait(player, SOTO.CharacterTrait.ALCOHOLIC);
...
local function SOTOAlcoholicEveryMinute()
	...
	SOTOalcoholicEffects(player, ticks)
...
local function SOTOAlcoholicEveryHour()
	...
	if player:hasTrait(SOTO.CharacterTrait.FORMER_ALCOHOLIC) then
		modData.FormerAlcoholicDaysSober = (modData.FormerAlcoholicDaysSober or 0) + 1;

		if modData.FormerAlcoholicDaysSober >= 24 then
			modData.FormerAlcoholicDaysSober = 0;
			modData.FormerAlcoholicRelapseChance = math.max(5, (modData.FormerAlcoholicRelapseChance or 100) - 1);
		end
	end
end
