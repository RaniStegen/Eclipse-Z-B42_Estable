...
CEWeaponDurabilityAlert.updateIntervalMs = 100
CEWeaponDurabilityAlert.lastUpdateTime = 0
...
function CEWeaponDurabilityAlert.update()
    ...
    -- Keep hide timing responsive even when weapon polling is throttled.
    if CEDurabilityIcon.instance and CEDurabilityIcon.instance.item and CEWeaponDurabilityAlert.hideTimer then
        if now >= CEWeaponDurabilityAlert.hideTimer then
            CEDurabilityIcon.instance:setItem(nil)
            CEWeaponDurabilityAlert.hideTimer = nil
        end
    end

    -- Poll weapon condition at a lower rate; per-frame checks are unnecessary here.
    if CEWeaponDurabilityAlert.lastUpdateTime > 0 and (now - CEWeaponDurabilityAlert.lastUpdateTime) < CEWeaponDurabilityAlert.updateIntervalMs then
        return
    end
    CEWeaponDurabilityAlert.lastUpdateTime = now
