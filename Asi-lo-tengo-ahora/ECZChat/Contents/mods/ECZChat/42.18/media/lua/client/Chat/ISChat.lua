...
ISChat.maxLine = ECZChatConfig.MAX_VISIBLE_LINES;
...
rpName = ECZChatState.getRPName() or "Superviviente"
print("[EclipseChat B42] ISChat.lua loaded")
...
function ISChat:initialise()
    ...
    self.pin = true;
    self.borderColor = { r = 0.62, g = 0.055, b = 0.070, a = 0.78 };
	rpName = ECZChatState.getRPName() or "Superviviente";
end
...
ISChat.initChat = function()
    ...
    if instance.tabCnt == 1 and instance.chatText then
        instance.chatText:setVisible(false);
        instance:removeChild(instance.chatText);
        instance.chatText = nil;
    elseif instance.tabCnt > 1 and instance.panel then
...
function ISChat:createChildren()
    ...
    self.textEntry.borderColor = {r=0.62, g=0.055, b=0.070, a=0.78};
...
function ISChat:onGearButtonClick()
    ...
    local authorsText = ECZChatState.getShowAuthors() and getText("UI_ECZChat_HideUsernames") or getText("UI_ECZChat_ShowUsernames")
    local authorsOption = context:addOption(authorsText, ISChat.instance, ISChat.onToggleAuthors)
    context:setOptionChecked(authorsOption, ECZChatState.getShowAuthors())
    ...
    if ECZChatV2UI and ECZChatV2UI.addSettings then ECZChatV2UI.addSettings(context, self) end
end
...
function ISChat:logChatCommand(command)
    ...
    if command and command ~= "" then
        ...
        for i,v in ipairs(self.chatText.log) do
            ...
            if i >= ECZChatConfig.MAX_COMMAND_HISTORY then
                break;
            end
        end
...
function ISChat:onCommandEntered()
    ...
    for streamIndex, stream in ipairs(ISChat.allChatStreams) do
        chatCommand = nil;
        ...
        if chatCommand then
            ...
            chat.chatText.streamID = streamIndex;
            if ECZChatV2UI and ECZChatV2UI.refreshChannelButton then ECZChatV2UI.refreshChannelButton(chat) end
    ...
    if not commandProcessed then
        local canSend, rateMessage = ECZChatState.canSend(chatStreamName, command)
        if not canSend then
            chat.servermsg = rateMessage
            chat.servermsgTimer = 3000
            return
        end
        ...
        if string.len(command) >= ECZChatConfig.MAX_MESSAGE_LENGTH then
            chat.servermsg = getText("UI_ECZChat_MessageTooLong")
            chat.servermsgTimer = 3000
            return;
        end
        -- .
        -- grammar
        local verb = getText("UI_ECZChat_VerbSays");
        -- .
        if chatStreamName == "gritar" then
            if luautils.stringStarts(command, " ") then
                command = command:sub(2);
            end
            local combined = getText("UI_ECZChat_Shouts", rpName, command);
        ...
        elseif chatStreamName == "faccion" then
            ...
            local combined = getText("UI_ECZChat_FactionRadio") .. " " .. rpName .. " " .. verb .. " ''" .. command .. "''";
            command = combined;
        ...
        elseif chatStreamName == "safehouse" then
            ...
            local combined = getText("UI_ECZChat_SafehouseRadio") .. " " .. rpName .. " " .. verb .. " ''" .. command .. "''";
            command = combined;
        ...
        elseif chatStreamName == "me" then
            local combined = "*0,255,255*" .. rpName .. command;
            command = combined;
            processSayMessage(command);
        -- .
        ...
        elseif chatStreamName == "name" then
            local valid, assignedName, reason = ECZChatState.setRPName(command)
            if valid then
                rpName = assignedName
                chat.servermsg = getText("UI_ECZChat_NameAssigned", assignedName)
            else
                chat.servermsg = reason
            end
            chat.servermsgTimer = 3500;
		-- for when we want to specify we are not speaking in-character. can also use /l
        elseif chatStreamName == "looc" then
            local combined = "*0,128,128*" .. rpName .. ": LOOC:" .. command .. "";
            command = combined;
            processSayMessage(command);
        -- .
        ...
        elseif chatStreamName == "do" then
            local combined = "*255,165,0*" .. command;
        command = combined;
        processSayMessage(command);
        -- Messi .
        end
...
ISChat.onSwitchStream = function()
    if ISChat.focused then
        ...
        if ECZChatV2UI and ECZChatV2UI.refreshChannelButton then ECZChatV2UI.refreshChannelButton(ISChat.instance) end
    end
end
...
function ISChat:updateChatPrefixSettings()
    ...
    for tabNumber, chatText in ipairs(self.tabs) do
        ...
        for i,msg in ipairs(chatText.chatMessages) do
            ...
            local author = msg:getAuthor()
            if not ECZChatState.getShowAuthors() and author then
                line = line:gsub("%[" .. author .. "%]" .. "%:", "")
            end
            line = line .. " <LINE> "
...
ISChat.onToggleAuthors = function()
    local chat = ISChat.instance;
    ECZChatState.toggleShowAuthors();
    chat:updateChatPrefixSettings();
end
...
ISChat.addLineInChat = function(message, tabID)
    ...
    local user = message:getAuthor();
    if not ECZChatState.getShowAuthors() and user then
        line = line:gsub("%[" .. user .. "%]" .. "%:", "");
    end
    ...
    if user and ECZChatState.isMuted(user) then return end
    ...
    if not chatText then
        chatText = ISChat.instance.chatText or ISChat.instance.defaultTab;
    end
    if not chatText then
        return;
    end
    ...
    while #chatText.chatMessages > ECZChatConfig.MAX_STORED_MESSAGES do table.remove(chatText.chatMessages, 1) end
    chatText:paginate();
...
ISChat.onToggleChatBox = function(key)
    ...
    local chat = ISChat.instance;
    if key == getCore():getKey("Switch chat stream") and chat.tabCnt and chat.tabCnt > 0 then
...
ISChat.createChat = function()
    ...
    if ISChat.instance ~= nil then
        print("[EclipseChat B42] Chat instance already exists; skipping duplicate creation")
        return;
    end
    ...
    if Events.SwitchChatStream then
