...
local function isLootChannelEnabled(key, defaultValue)
    local value = sandboxWeapons["Enable_" .. key]
    if value == nil then return defaultValue end
    return value == true
end

local function weaponsInContainersEnabled()
    return isLootChannelEnabled("WeaponsInContainers", true)
end

local function weaponsOnZombiesEnabled()
    return isLootChannelEnabled("WeaponsOnZombies", false)
end

local function ammoInContainersEnabled()
    return isLootChannelEnabled("AmmoInContainers", true)
end

local function attachmentsInContainersEnabled()
    return isLootChannelEnabled("AttachmentsInContainers", true)
end

local function explosivesOnZombiesEnabled()
    return isLootChannelEnabled("ExplosivesOnZombies", false)
end

local function weapon(item, baseItem, chance)
    if not weaponsInContainersEnabled() or not isSandboxEnabled(item) then return end
    -- Distribution.Insert derives the new entry from its vanilla counterpart. This
    -- deliberately keeps Guns of Marz tied to the game's vanilla loot abundance.
    Distribution.Insert(baseItem, chance, tables, "MarzGuns." .. item)
end

local function ammo(item, baseItem, chance)
    if not ammoInContainersEnabled() then return end
    if not isSandboxEnabled(item:gsub("_Box", ""):gsub("_Carton", "")) then return end
    Distribution.Insert(baseItem, chance, tables, "MarzGuns." .. item)
end

local function zombie(item, baseItem)
    if not weaponsOnZombiesEnabled() or not isSandboxEnabled(item) then return end
...
local function applyDistributionPass()
    ...
    if attachmentsInContainersEnabled() then
    ...
    if attachmentsInContainersEnabled() then
        ...
        Distribution.InsertMany("Base.Laser", 0.05, tables, "MarzGuns.MKI_Suppressor", "MarzGuns.NDR_Suppressor", "MarzGuns.PBS-1_Suppressor", "MarzGuns.Shh9_Suppressor", "MarzGuns.MP_Suppressor", "MarzGuns.P45_Suppressor")
        ...
        Distribution.InsertMany("Base.RedDot", 0.05, tables, "MarzGuns.MKI_Suppressor", "MarzGuns.NDR_Suppressor", "MarzGuns.PBS-1_Suppressor", "MarzGuns.Shh9_Suppressor", "MarzGuns.MP_Suppressor", "MarzGuns.P45_Suppressor")
        ...
        Distribution.InsertMany("Base.GunLight", 0.05, tables, "MarzGuns.MKI_Suppressor", "MarzGuns.NDR_Suppressor", "MarzGuns.PBS-1_Suppressor", "MarzGuns.Shh9_Suppressor", "MarzGuns.MP_Suppressor", "MarzGuns.P45_Suppressor")
    ...
    -- Preserve vanilla loot for every disabled channel. When a channel is
    -- enabled, GoM variants inherit the vanilla distribution weights before the
    -- corresponding vanilla entries are removed.
    if weaponsInContainersEnabled() then
        Distribution.RemoveMany(tables, unpack(vanillaWeaponItems))
    end
    if ammoInContainersEnabled() then
        Distribution.RemoveMany(tables, unpack(vanillaAmmoItems))
    end
    if attachmentsInContainersEnabled() then
        Distribution.RemoveMany(tables, unpack(vanillaAttachmentItems))
    end
end
...
Events.OnInitGlobalModData.Add(
    function()
        ...
        if explosivesOnZombiesEnabled() and sandboxWeapons.Enable_Explosives then
        ...
        if weaponsOnZombiesEnabled() then
            ...
            Distribution.RemoveMany(zombieTables, unpack(vanillaWeaponItems))
        end
    end
)
