...
if not IDBFS._mpHandcraftPatchApplied then
    ...
    local function addCount(counts, item)
        local fullType = getFullType(item)
        if fullType then
            counts[fullType] = (counts[fullType] or 0) + IDBFS.getItemCount(item)
        end
    end
    ...
    local function inferPressRecipe(items)
        ...
        local sugarRecipe = IDBFS.PressRecipes and IDBFS.PressRecipes.sugar or nil
        local sugarCount = 0
        eachItem(items, function(item)
            if sugarRecipe and IDBFS.matchesIngredient(item, sugarRecipe.ingredients[1]) then
                sugarCount = sugarCount + IDBFS.getItemCount(item)
            end
        end)
        if sugarCount >= 1 then
    ...
    local function getRecipeJobName(recipeKey)
        ...
        if recipe and recipe.liquidType and IDBFS.getLiquidLabel then
            return tostring(IDBFS.getLiquidLabel(recipe.liquidType))
        end
        if getText then
            local translated = getText("IGUI_CraftingWindow_IDBFSPress")
            if translated and translated ~= "IGUI_CraftingWindow_IDBFSPress" then
                return translated
            end
        end
