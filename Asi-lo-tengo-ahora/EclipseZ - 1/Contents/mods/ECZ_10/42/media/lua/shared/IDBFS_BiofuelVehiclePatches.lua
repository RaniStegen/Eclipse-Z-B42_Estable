...
local function getVehicleFuelType(part)
    local amount = part and part.getContainerContentAmount
        and tonumber(part:getContainerContentAmount()) or 0
    if amount <= 0 then
        return nil
    end
    local modData = part and part.getModData and part:getModData() or nil
    return modData and modData.IDBFSFuelType or "petrol"
end
...
local function patchAddGasolineToVehicle()
    ...
    ISAddGasolineToVehicle.complete = function(self)
        ...
        local existingFuelType = getVehicleFuelType(self.part)
        if existingFuelType and existingFuelType ~= fuelType then
            print("[IDBFS][WARN] Refusing to mix " .. tostring(fuelType)
                .. " with " .. tostring(existingFuelType) .. " in a vehicle tank")
            return false
        end
...
local function patchAddFuelToGenerator()
    ...
    ISAddFuel.complete = function(self)
        ...
        local existingFuel = self.generator and self.generator.getFuel
            and tonumber(self.generator:getFuel()) or 0
        local generatorData = self.generator and self.generator.getModData
            and self.generator:getModData() or nil
        local existingFuelType = existingFuel > 0
            and (generatorData and generatorData.IDBFSFuelType or "petrol") or nil
        if existingFuelType and existingFuelType ~= fuelType then
            print("[IDBFS][WARN] Refusing to mix " .. tostring(fuelType)
                .. " with " .. tostring(existingFuelType) .. " in a generator")
            return false
        end
        local result = originalComplete(self)
...
local function registerBiofuelGeneratorsOnSquare(square)
    ...
    for i = 0, objects:size() - 1 do
        ...
        if instanceof then
            local ok, result = pcall(instanceof, object, "IsoGenerator")
            isGenerator = ok and result == true
        else
            isGenerator = object and object.getFuel and object.getModData and object.setFuel
        end
        if isGenerator then
            local modData = object:getModData()
            local fuel = tonumber(object:getFuel()) or 0
            if modData and modData.IDBFSFuelType == "biofuel" and fuel > 0 then
                if not tonumber(modData.IDBFSLastFuel) then
                    modData.IDBFSLastFuel = fuel
                end
                registerBiofuelGenerator(object)
            end
        end
    end
end
...
addEvent(Events.LoadGridsquare, registerBiofuelGeneratorsOnSquare)
