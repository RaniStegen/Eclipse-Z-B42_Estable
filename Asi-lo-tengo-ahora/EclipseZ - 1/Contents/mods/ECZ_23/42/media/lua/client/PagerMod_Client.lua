...
function PagerMod.deployTower(item)
    ...
    -- Spawn the solid world object. Its MAIN sprite is the invisible base (so it
    -- is a valid, persistent, collidable object); the visible tower art is added
    -- as an attached anim by applyTowerVisual. If the custom art's texture failed
    -- to load, fall back to a real vanilla sprite so it's never invisible/culled.
    if PagerMod.registerTowerSprites then PagerMod.registerTowerSprites() end
    local hasArt = PagerMod.towerSpriteReady and PagerMod.towerSpriteReady() or false
    local cell = getCell()
    local baseName = hasArt and PagerMod.TOWER_BASE_SPRITE or "location_shop_zippee_01_56"
    local obj = IsoThumpable.new(cell, square, baseName, false, nil)
    if obj.setName then obj:setName(PagerMod.TOWER_OBJECT_NAME) end
    if obj.setMaxHealth then obj:setMaxHealth(1000) end
    if obj.setHealth then obj:setHealth(1000) end
    -- Guarantee collision regardless of the sprite's own flags.
    if obj.setBlockAllTheSquare then obj:setBlockAllTheSquare(true) end
    if obj.setCanPassThrough then obj:setCanPassThrough(false) end
    local omd = obj:getModData()
    omd.pagerTower = true
    -- Facing: 0 = up/down sprite, 1 = left/right sprite, from the way the player
    -- is looking when they set it up.
    omd.towerDir = 0
    pcall(function()
        local d = p:getDir()
    ...
    if hasArt and PagerMod.applyTowerVisual then PagerMod.applyTowerVisual(obj) end
    -- Add via transmitAddObjectToSquare (adds locally + syncs in MP) so the
    -- object lands in the square's normal object list and is reliably picked up
    -- by right-click context menus (AddSpecialObject objects often are not).
    ...
    -- Consume the carried tower item.
