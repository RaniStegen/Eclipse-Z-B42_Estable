...
function ISWorldMap:new(x, y, width, height)
	...
	if SandboxVars and SandboxVars.SDGlobalRewards then
		...
		for i=1,#gRewardTypes do
			...
			local sandboxString = "  "
			for j=1,#split do
				local sItem = ScriptManager.instance:getItem(split[j])
				if sItem then
					local displayName = sItem:getDisplayName()
					local color = red
...
function ISWorldMap:onRightMouseUp(x, y)
	...
	local tier, zone = checkZone()
	local _tier, _zone = checkZoneAtXY(worldX, worldY) --zone at mouse coordinates
	if _zone == "CC" and zone == "CC" then --if player is in CC and the mouse is hovering over CC
		context:addOptionOnTop("Teleport to this location at CC: " .. worldX .. "," .. worldY .. ",0", self, 
			function()
				playerObj:setX(worldX)
				playerObj:setY(worldY)
				playerObj:setZ(0)
	...
	if getAccessLevel() == "Overseer" or getAccessLevel() == "Moderator" or getAccessLevel() == "GM" then
	...
	if isDebugEnabled() or getAccessLevel() == "Overseer" or getAccessLevel() == "Moderator" or getAccessLevel() == "GM" or getAccessLevel() == "admin" then
		local newmapzone 	= context:addOptionOnTop("Draw new map zone:", self, nil)
		local submenu 	= ISContextMenu:getNew(context)
		context:addSubMenu(newmapzone, submenu)
		
		context:addOptionOnTop("Open Master Zone Manager", self, 
			function()
				local manager = SDmap.ZoneManagerPanel:new(getCore():getScreenWidth()/2 - 450, getCore():getScreenHeight()/2 - 300, 900, 600, self.mapAPI)
				manager:initialise()
			...
			submenu2:addOptionOnTop("DELETE: " .. zinfo, self,
				function()
					...
					for i=symbolsAPI:getSymbolCount()-1, 0, -1 do
						...
						if symbol.getSymbolID and symbol:getSymbolID() == "Asterisk" then
							symbolsAPI:removeSymbolByIndex(i)
						end
					end
...
local MapMarkerSystem = rawget(_G, "MapMarkerSystem") or { FontList = { "Small" } };
