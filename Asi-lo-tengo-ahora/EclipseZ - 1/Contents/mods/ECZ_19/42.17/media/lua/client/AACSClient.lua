...
local function _makeOptionToolTip(text)
...
local function _refreshLastLoginViews()
...
function AACS.OnServerCommand(module, command, args)
    ...
    if command == "updateClientLastLogin" then
        ...
        if u and ts then
            ...
            _refreshLastLoginViews()
        end
        return
    end

    if command == "lastLoginSnapshotData" then
        AACS.PlayerLastLoginDB = args.entries or {}
        _refreshLastLoginViews()
        return
    end
    ...
    if command == "myAnimalsData" then
        ...
        for i = 1, #entries do
            entries[i] = AACS.EnsureEntrySchema(entries[i])
        end
        AACS.ClientCache.myEntries = entries
    ...
    if command == "interactableAnimalsData" then
        ...
        AACS.ClientCache.interactEntries = entries
        if AACS.UI.UserManager and AACS.UI.UserManager.updateFromCache then
            AACS.UI.UserManager:updateFromCache()
        end
        return
    ...
    if command == "allAnimalsData" then
        ...
        AACS.ClientCache.allEntries = entries
    ...
    if command == "updateEntry" then
        local e = AACS.EnsureEntrySchema(args.entry)
...
_openUserManager = function()
    ...
    if isClient() then
        sendClientCommand(getPlayer(), "AACS", "requestLastLoginData", {})
            ...
            if not alreadyProcessed then
                ...
                if uid then
                     ...
                     if sub.options then
                        ...
                        _iterList(sub.options, function(sopt)
                            ...
                            if not canLeash and matchesAny(sopt.name, leashKeys) then
                                ...
                                sopt.toolTip = _makeOptionToolTip(getText("IGUI_AACS_Notify_Protected", owner, uid))
                            end
                    ...
                    if not canAdopt then
                         ...
                         if blockReason then
                            adoptOpt.toolTip = _makeOptionToolTip(blockReason)
                         end
                    end
                end
            end
...
function AACS._sendLoginPingOnce()
    ...
    sendClientCommand(p, "AACS", "requestLastLoginData", {})
    sendClientCommand(p, "AACS", "requestMyAnimals", {})
    sendClientCommand(p, "AACS", "requestInteractableAnimals", {})
    if AACS.IsAdmin(p) then
        sendClientCommand(p, "AACS", "requestAllAnimals", {})
    end
    Events.OnTick.Remove(AACS._sendLoginPingOnce)
end
