...
function ISVehiclePartMenu.getPropaneTankNotFull(playerObj, typeToItem)
	...
	if equipped and (equipped:getType() == "PropaneTank" or equipped:getType() == "BlowTorch") and item:getCurrentUses() < item:getMaxUses() then
		return equipped
	...
	if typeToItem["Base.PropaneTank"] then
		...
		for _,item in ipairs(typeToItem["Base.PropaneTank"]) do
			if item:getCurrentUses() < item:getMaxUses() and item:getCurrentUses() > usedDelta then
				gasCan = item
				usedDelta = gasCan:getCurrentUses()
			end
		end
		if gasCan == nil then
			for _,item in ipairs(typeToItem["Base.BlowTorch"]) do
				if item:getCurrentUses() < item:getMaxUses() and item:getCurrentUses() > usedDelta then
					gasCan = item
					usedDelta = gasCan:getCurrentUses()
				end
...
function ISVehiclePartMenu.onTakePropane(playerObj, part)
	...
	if item then
		...
		ISTimedActionQueue.add(FR_TakePropaneAction:new(playerObj, part, item, 120))
	end
end
