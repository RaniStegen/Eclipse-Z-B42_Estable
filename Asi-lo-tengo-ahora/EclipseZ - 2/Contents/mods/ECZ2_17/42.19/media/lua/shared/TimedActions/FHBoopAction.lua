...
function FHBoopAction:waitToStart()
	if self.character:isSeatedInVehicle() then
		-- B42 FIX: don't forceComplete() from waitToStart(), the action has not
		-- started yet. start() bails out instead, which is the vanilla pattern.
		return false
...
function FHBoopAction:start()
	-- B42 FIX: pushing an action anim onto a seated character clobbers the
	-- vehicle passenger animation, which then stalls the door action queued
	-- after us (fatal in MP, where ISCloseVehicleDoor:isValid() never drops it).
	if self.character:isSeatedInVehicle() then
		self:forceComplete()
		return
	end
